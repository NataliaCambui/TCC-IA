...
# -*- coding: utf-8 -*-
"""
Unified file: ai_benchmark_v32e_fixed.py

Fixes:
- Executor payload now uses {"code_string","function_name","args"}.
- If "args" is missing or empty, fills sensible defaults based on function_name
  (so o Executor não chama a função sem parâmetros).
- Mantém compatibilidade com o restante do v32e (bateria original intacta).

Como usar:
  python ai_benchmark_v32e_fixed.py [seus argumentos do v32e]
"""

import json, types, functools, logging, os
import requests

# --- Defaults por função (fallback quando args vier vazio) ---
_DEFAULT_ARGS_BY_FUNCTION = {
    "fibonacci":        [10],
...
# Aceita sinônimos comuns de nomes de função
_FN_ALIASES = {
    "fibonacci_sequence": "fibonacci",
    "valid_parentheses": "is_valid_parentheses",
    "palindrome_check": "is_palindrome",
    "count_islands_in_grid": "count_islands",
}
...
def _normalize_fn_name(name: str) -> str:
    if not isinstance(name, str):
        return ""
    key = name.strip()
    return _FN_ALIASES.get(key, key)

# --- Patch: adequar payload do executor e preencher args quando necessário ---
_requests_post_original = requests.post

def _patched_requests_post(url, *args, **kwargs):
    # Só mexe em chamadas para /execute
    if isinstance(url, str) and "/execute" in url:
        json_payload = kwargs.get("json")
        if isinstance(json_payload, dict):
            # Corrigir chaves esperadas pelo seu Executor
            if "code_string" not in json_payload and "code" in json_payload:
                json_payload["code_string"] = json_payload.pop("code")
            if "function_name" not in json_payload:
                # aceitar 'func_name' ou 'entrypoint'
                if "func_name" in json_payload:
                    json_payload["function_name"] = json_payload.pop("func_name")
                elif "entrypoint" in json_payload:
                    json_payload["function_name"] = json_payload.pop("entrypoint")

            # Garantir 'args' como lista
            if "args" not in json_payload:
                # Alguns formatos antigos: tests:[{"func","args"}]
                if "tests" in json_payload and isinstance(json_payload["tests"], list) and json_payload["tests"]:
                    first = json_payload["tests"][0]
                    if isinstance(first, dict) and "args" in first:
                        json_payload["args"] = first["args"]
                else:
                    json_payload["args"] = []

            # Fallback: se args estiver vazio, preencher pelo function_name
            fn = _normalize_fn_name(json_payload.get("function_name", ""))
            if isinstance(json_payload.get("args"), list) and len(json_payload["args"]) == 0 and fn:
                default_args = _DEFAULT_ARGS_BY_FUNCTION.get(fn)
                if default_args is not None:
                    json_payload["args"] = default_args

            # Remover chaves de outros esquemas para não confundir o servidor
            for k in ("tests", "language", "source", "inputs"):
            ...
            logging.getLogger("bench").debug(f"[executor] final payload: {json_payload}")

    return _requests_post_original(url, *args, **kwargs)

# Ativa o patch globalmente
requests.post = _patched_requests_post

# =======================
#  Conteúdo original v32e
# =======================
...
class AIBenchmark:
    ...
    def __init__(
        ...
        self.gemini_model = (
            (
                gemini_model_name
                or os.getenv("GEMINI_MODEL_NAME", "gemini-pro-latest")
            )
        ...
        if can_exec:
            if challenge["name"] == "LRU Cache":
                ...
                if (
                    ...
                    # Calcula tempo total da sequência, não a média, para ser comparável
                    all_times = [s["t_ms"] for s in raw_results if s.get("t_ms", -1) >= 0]
                    total_time_ms = np.sum(all_times) if all_times else -1.0
                    ...
                    details_curr_run.append(
                        {
                            ...
                            "execution_time_ms": total_time_ms,  # Usa o tempo TOTAL
    ...
    def _calculate_aggregated_test_metrics(self) -> dict:
        ...
        try:
            ...
            for name, group in grouped:
                ...
                # =======================================================
                challenge_name = name[1] # Pega o nome do desafio da chave do grupo
                ...
                # LÓGICA CONDICIONAL PARA CALCULAR O TEMPO
                if challenge_name == "LRU Cache":
                    # Para LRU, o tempo já é o total da sequência, então fazemos a média desses totais entre os runs
                    avg_time = valid_times.mean() if not valid_times.empty else np.nan
                else:
                    # Para outros, fazemos a média dos tempos de cada teste individual
                    avg_time = valid_times.mean() if not valid_times.empty else np.nan
    ...
    def _plot_bar_charts(self, df_plot, ai_list, cmap, challenge_order, ts, out_dir):
        ...
        for key, (title, ylabel, col_name) in metrics_to_plot.items():
            ...
            try:
                ...
                #           *** INÍCIO DA MODIFICAÇÃO ***
                # =======================================================
                # Adiciona escala logarítmica APENAS ao gráfico de tempo de execução
                if col_name == "avg_exec_time_ms":
                    ax.set_yscale('log')
                ...
                if col_name in ["completeness", "avg_exec_success_rate"]:
                    ...
                    ax.set_ylim(bottom=0.001) # Ajusta limite inferior para escala log
    ...
    def _plot_lru_times_chart(self, detailed_df, ai_list, cmap, ts, out_dir):
        """Gera gráfico de barras para o tempo de operação do LRU Cache (média sobre runs)."""
        logging.info("Gerando Gráfico de Tempo - LRU Cache (média sobre runs)...")
        ...
        # Agrupa por AI e RUN para somar os tempos de cada sequência, DEPOIS calcula a média das somas
        total_time_per_run = lru_data.groupby(["ai", "run_index"])["execution_time_ms"].sum()
        avg_total_time_per_ai = total_time_per_run.groupby("ai").mean()
        avg_total_time_per_ai = avg_total_time_per_ai.reindex(ai_list).dropna()

        if avg_total_time_per_ai.empty:
            logging.warning(
                f"Pulando gráfico LRU Times: Sem dados válidos após agrupar/reindexar por IA."
            )
            return

        fig, ax = plt.subplots(figsize=(max(8, len(avg_total_time_per_ai) * 1.5), 6))
        try:
            colors_for_plot = [
                cmap.get(ai, "grey") for ai in avg_total_time_per_ai.index
            ]
            avg_total_time_per_ai.plot(
                kind="bar", ax=ax, color=colors_for_plot, width=0.6
            )

            ax.set_title(
                f"Tempo Total da Sequência - {challenge_name} (Média sobre {self.num_runs} runs)"
            )
            ax.set_ylabel("Tempo Total da Sequência (ms)")
    ...
    def create_visualizations(self, df_agg, ts, out_dir):
        ...
        detailed_df_for_lru = None
        if self.detailed_test_results:
            try:
                detailed_df_for_lru = pd.DataFrame(self.detailed_test_results)
                detailed_df_for_lru["execution_time_ms"] = pd.to_numeric(
                    detailed_df_for_lru["execution_time_ms"], errors="coerce"
                )
        ...
        if detailed_df_for_lru is not None:
           self._plot_lru_times_chart(detailed_df_for_lru, ais_in_data, cmap, ts, out_dir)
