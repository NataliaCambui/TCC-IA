...
# -*- coding: utf-8 -*-
"""
ai_benchmark_v33c_executor_payload_autodetect.py

Adds:
- Executor payload auto-detection: tries multiple common schemas when a 400/422 occurs.
- Logs response body (first 1k chars) on non-2xx to help debugging.
- Preserves per-provider concurrency, retries with backoff + jitter, and metrics.

Schemas attempted for Executor POST /execute (in order):
  S1) {"code_string": <str>, "func_name": <str>, "args": <list>}
  S2) {"code": <str>, "entrypoint": <str>, "args": <list>}
  S3) {"code": <str>, "tests": [{"func": <str>, "args": <list>}]}  # expects one test
  S4) {"language": "python", "source": <str>, "function": <str>, "inputs": <list>}

The first schema that returns 2xx is used.
"""
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Benchmark com controle de concorrência e autodetecção de payload do Executor.")
    p.add_argument("--num_runs", type=int, default=2)
    p.add_argument("--output_dir", type=str, default="benchmark_reports")
    ...
    p.add_argument("--requests_timeout", type=float, default=float(os.getenv("REQUESTS_TIMEOUT", "60")))
    p.add_argument("--max_concurrency", type=int, default=int(os.getenv("MAX_CONCURRENCY","8")))
    p.add_argument("--max_concurrency_openai", type=int, default=int(os.getenv("MAX_CONCURRENCY_OPENAI","0")))
    p.add_argument("--max_concurrency_gemini", type=int, default=int(os.getenv("MAX_CONCURRENCY_GEMINI","0")))
    p.add_argument("--max_concurrency_claude", type=int, default=int(os.getenv("MAX_CONCURRENCY_CLAUDE","0")))
    p.add_argument("--max_concurrency_executor", type=int, default=int(os.getenv("MAX_CONCURRENCY_EXECUTOR","0")))
...
class HttpCallResult:
    ...
    url: str
    provider: str
    body: Optional[str] = None  # response.text (truncated) for error cases

class HttpClient:
    def __init__(self, timeout: float, max_workers: int, max_conn_pool: int = 100, log: Optional[logging.Logger] = None):
    ...
    def post_json_with_backoff(self, url: str, payload: dict, headers: dict, provider: str, max_attempts: int = 5) -> HttpCallResult:
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                return HttpCallResult(True, status, (time.time()-t0_all)*1000.0, data, None, attempt, hits_429, url, provider, None)
            except Exception as e:
                last_err = str(e)
                delay = min(2 ** attempt, 16) + random.random()
                if self.log:
                    self.log.debug(f"[{provider}] POST {url} attempt={attempt+1}/{max_attempts} err={e} resp_body={last_body!r} sleep={delay:.2f}s")
                time.sleep(delay)
            finally:
                self._exit_sems(provider)
        return HttpCallResult(False, status, (time.time()-t0_all)*1000.0, data, last_err, max_attempts-1, hits_429, url, provider, last_body)
...
# ---------- Executor with schema auto-detect ----------

def _executor_payload_variants(code: str, func_name: str, args: list) -> List[dict]:
    return [
        {"code_string": code, "func_name": func_name, "args": args},                                   # S1
        {"code": code, "entrypoint": func_name, "args": args},                                         # S2
        {"code": code, "tests": [{"func": func_name, "args": args}]},                                  # S3
        {"language": "python", "source": code, "function": func_name, "inputs": args},                 # S4
    ]

def run_executor_code(http: HttpClient, executor_url: str, code: str, func_name: str, args: list) -> Dict[str, Any]:
    headers = {"Content-Type":"application/json"}
    body_preview = None
    for i, payload in enumerate(_executor_payload_variants(code, func_name, args), start=1):
        ...
        if res.ok:
            ...
            body_preview = res.body
            # If 400/422, try next schema
            if res.status not in (400, 422):
                # Other errors likely not schema-related; stop early
                break
    # If we got here, all schema attempts failed
    return {
        "provider":"executor",
        "schema_variant": None,
        "ok": False,
        ...
        "retries": 0,
        "hits_429": 0,
        "error": f"All executor schema variants failed. Last body: {body_preview!r}"
    }

# (Optional) Gemini via SDK
...
def run_benchmark(args) -> Dict[str, Any]:
    ...
    if args.max_concurrency_openai > 0: http.set_provider_limit("openai", args.max_concurrency_openai)
    if args.max_concurrency_gemini > 0: http.set_provider_limit("gemini", args.max_concurrency_gemini)
    if args.max_concurrency_claude > 0: http.set_provider_limit("claude", args.max_concurrency_claude)
    if args.max_concurrency_executor > 0: http.set_provider_limit("executor", args.max_concurrency_executor)
    ...
    log.info(f"Concorrência global={args.max_concurrency} (prov: openai={args.max_concurrency_openai} gemini={args.max_concurrency_gemini} claude={args.max_concurrency_claude} executor={args.max_concurrency_executor})")
    ...
    detail_path = Path(args.output_dir) / f"detail_{int(time.time())}.json"
    summary_path = Path(args.output_dir) / f"summary_{int(time.time())}.json"
    detail_path.write_text(json.dumps({"results": results}, ensure_ascii=False, indent=2), encoding="utf-8")
