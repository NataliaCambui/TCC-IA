        ...
        for key, (title, ylabel, col_name) in metrics_def.items():
            ...
            try:
                ...
                num_ais=len(ai_list)
                        ...
                        # ... (código anterior dentro do loop for key, (title, ylabel, col_name)...)
                if col_name == 'avg_exec_time_ms': # Adiciona esta condição para imprimir só para este gráfico
                    print(f"\n--- Dados para Gráfico: {title} ---")
                    print(pivot_table) # Imprime a tabela dinâmica usada para o plot
                    print("------\n")

                if pivot_table.isnull().all().all(): print(f"Skip bar '{title}': NaN."); plt.close(); continue

                # Linha original para plotar o gráfico:
                ax = pivot_table.plot(kind='bar', figsize=(max(12, num_ais * 2), 7),
                                    color=[cmap.get(ai, 'grey') for ai in pivot_table.columns], width=0.8)
                # ... (resto do código do plot) ...
