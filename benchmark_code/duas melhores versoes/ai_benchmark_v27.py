...
class AIBenchmark:
    ...
    def query_claude(self, prompt: str) -> dict:
        ...
        try:
            ...
            err_type = (
                ...
                else "(Server Error)" if status_code >= 500 else "(Network/Other Error)"
            )
            if e.response is not None:
                err_msg += f" | Status:{status_code}{err_type}"
            try:
                err_msg += f" | Body:{e.response.json()}"
            except:
                err_msg += f" | Body:{e.response.text[:100]}..."
            print(err_msg)
        except Exception as e:
    ...
    def query_chatgpt(self, prompt: str) -> dict:
        ...
        try:
            ...
            try:
                ...
                err_msg += f" | API Err:{api_err}" if api_err else f" | Body:{details}"
            except:
                err_msg += f" | Body:{e.response.text[:100]}..."
            print(err_msg)
        except Exception as e:
    ...
    def measure_response_completeness(self, code: str, description: str) -> int:
        ...
        if lines > 0:
            ...
            if "fibonacci" in dl and (
                ...
                "dict" in cl or "capacity" in cl or "OrderedDict" in code
            ):
                bonus = 10
            score += bonus
            struct = 0
        ...
        try:
            ...
            if res.returncode in [0, 1] and res.stdout:
                try:
                    ...
                    return len(res.stdout.strip().splitlines())  # Fallback
            elif res.stderr:
        ...
        try:
            ...
            if not class_name:
                ...
                if match:
                    ...
                    raise NameError("Nome classe LRUCache não encontrado.")
            LruClass = local_namespace.get(class_name)
            if not LruClass:
                LruClass = safe_globals.get(class_name)  # Tenta globals
                if not LruClass:
                    raise NameError(f"Classe '{class_name}' não definida.")
                    ...
                    if operation == "__init__":
                        ...
                        if not hasattr(lru_instance, operation):
                            raise AttributeError(
                                f"Instância não tem método '{operation}'"
                            )
                        ...
                        if operation == "put":
                            ...
                            if str(output) == str(expected):
                                ...
                                step_res["err"] = f"Out:'{output}'!=Exp:'{expected}'"
                                final_success = False
                    else:
    ...
    def _execute_lru_locally(self, code_string: str, test_sequence: list) -> dict:
        ...
        try:
            ...
            if process.is_alive():
                print(
                    f"  AVISO: Timeout ({timeout_seconds}s) processo LRU. Terminando."
                )
                process.terminate()
                process.join(1)
                if process.is_alive():
            ...
            if not terminated:
                ...
                if exit_code == 0:
                    try:
                        ...
                        print(
                            f"  Exec LRU local OK ({elapsed_time:.2f}s). Worker success: {result.get('success')}"
                        )
                        return_value = result
                    except queue.Empty:
                        print("  ERRO: Processo LRU OK, mas fila vazia.")
                        return_value = {
                            "success": False,
                            "results": [],
                            "error": "Worker OK but no result",
                        }
                    ...
                    try:
                        ...
                        return_value = {
                            "success": False,
                            "results": [],
                            "error": f"Worker exit {exit_code}, q err: {e_qf}",
                        }
            ...
            return_value = {
                "success": False,
                "results": [],
                "error": f"Process mgmt error: {e_p}",
            }
            ...
            try:
                ...
                print(f"  Aviso: Erro limpar fila LRU: {e_qc}")
            try:
                if process:
                    process.close()
            except Exception as e_pc:
                print(f"  Aviso: Erro limpar processo LRU: {e_pc}")
        return return_value
    ...
    def evaluate_solution(self, ai_name: str, challenge: dict, response: dict) -> dict:
        ...
        if can_exec and challenge["name"] != "LRU Cache":
            func_name = self._detect_function_name(
                code, challenge
            )  # Passa challenge dict
            if not func_name:
        ...
        if can_exec:
            if challenge["name"] == "LRU Cache":
                    ...
                    if not first_err and not ok:
                        for step in raw_results:
                            if step.get("err"):
                                ...
                                break
                    if not first_err and not ok:
                        first_err = "LRU exec falhou (passo?)"  # Fallback
                    # Calcula tempo médio dos passos
                    t_ok = [s["t_ms"] for s in raw_results if s.get("t_ms", -1) >= 0]
                    ...
                    details_curr.append(
                        {
                            ...
                            "expected_output_str": "Ver Passos",
                            "actual_output_str": "Ver Passos",
                            "status": (
                            ...
                            "error_message": str(first_err)[
                                :250
                            ],  # Garante string e limita tamanho
                            "stdout": None,
                        }
                    )
                    print(f"  Resultado LRU local: {'Sucesso' if ok else 'Falha/Erro'}")
                else:  # Caso de teste LRU mal formatado
                    print("  AVISO: Casos teste LRU inválidos.")
                    n_errors = 1
                    first_err = "Caso teste LRU inválido."
                    details_curr.append(
                        {
                            "ai": ai_name,
                            "challenge": challenge["name"],
                ...
                print(
                    f"  Executando '{challenge['name']}' de {ai_name} via Executor Service..."
                )
                # Chama a função corrigida para executar testes externos
                raw, details, errors, first_e = self._run_test_cases_fixed(
                    ai_name, challenge, code, func_name
                )
                raw_results = raw
                n_errors = errors
                first_err = first_e
                details_curr.extend(details)
        else:  # Sem código ou nome função (e não é LRU)
            err_msg = "Código não extraído" if not code else "Nome func/classe N/A"
            print(
                f"  AVISO: {err_msg} p/ '{challenge['name']}' por {ai_name}. Pulando exec."
            )
            n_tests = len(
                challenge.get("test_cases", [])
            )  # Número de testes que seriam corridos
            n_errors = n_tests
            first_err = err_msg  # Marca todos como erro
    ...
    def _detect_function_name(self, code: str, challenge: dict) -> str | None:
        ...
        if first_match:
            ...
            if expected_name and found_name != expected_name:
                print(
                    f"  AVISO: Usando 1ª definição '{found_name}' (Esperado: '{expected_name}')."
                )
        ...
        print(
            f"  ERRO: Nenhuma definição func/classe encontrada p/ {challenge['name']}."
        )
        return None

    # --- CORREÇÃO v19: Nome da função corrigida ---
    def _run_test_cases_fixed(
        self, ai_name: str, challenge: dict, code: str, actual_function_name: str
        ...
        if not test_cases:
            print(f"  Aviso: Sem casos teste p/ '{challenge['name']}'.")
            return [], [], 0, ""
        ...
        for i, tc in enumerate(test_cases):
            ...
            if args is None:
                print(f"  Aviso: Caso {i+1} '{challenge['name']}' s/ 'input_args'.")
                continue
            ...
            if exec_res.get("success") is True:
                ...
                try:
                    ...
                    print(
                        f"  AVISO: Erro comparar outputs {ai_name}/{challenge['name']}/Caso {i}: {comp_e}"
                    )
                ...
                t_det["error_message"] = exec_res.get(
                    "error", "Executor sem status sucesso"
                )
                case_err = str(t_det["error_message"])[:250]
                n_fails_errors += 1
        ...
        try:
            ...
            if resp.status_code == 200:
                try:
                    ...
                    print(f"  Erro JSON executor: {json_e}")
            ...
            print(f"  Erro Timeout executor ({self.executor_url}).")
            default_err["error"] = "Executor timeout"
            return default_err
        except requests.exceptions.RequestException as req_e:
            print(f"  Erro conexão executor: {req_e}")
            default_err["error"] = f"Executor connection error: {req_e}"
            return default_err
        except Exception as e:
            print(f"  Erro inesperado executor: {type(e).__name__} - {e}")
            default_err["error"] = f"Executor unexpected error: {type(e).__name__}"
            return default_err
    ...
    def run_benchmark(self):
        ...
        if not enabled_ais:
            print("ERRO: Nenhuma API config.")
            return
        print(f"Iniciando Benchmark para: {', '.join(enabled_ais)}")
        print(
            f"  Claude:{self.claude_model}|ChatGPT:{self.openai_model}|Gemini:{self.gemini_model or 'N/A'}"
        )
        print(f"  LRU Exec: Local")
        print("=" * 42)
        ...
        for chal in self.code_challenges:
            sig = chal.get(
                "function_signature", f"{chal['name'].lower().replace(' ','_')}(...)"
            )
            ...
            print(
                f"  {ch_details.get(chal['name'],'Explicação N/A.')}"
            ...
            if can_gemini:
                ...
                resps["Gemini"] = self.query_gemini(prompt)
            ...
            for ai in enabled_ais:
                ...
                result = self.evaluate_solution(ai, chal, resp_data)
                self.results.append(result)
                n_err_fail = result.get("num_execution_errors", 0)
                det = [
                    d
                    for d in self.detailed_test_results
                    if d["ai"] == ai and d["challenge"] == chal["name"]
                ]
                n_att = len([d for d in det if d["status"] != "No Code"])
                ...
                if n_att == 0:
                    status = "Not Run/No Code"
                elif chal["name"] == "LRU Cache":
                    status = det[0]["status"] if det else "Error"
                else:
    ...
    def _prepare_dataframe(self) -> pd.DataFrame | None:
        ...
        try:
            ...
            df_clean = df.drop(
                ["execution_results_raw", "code", "full_response"],
                axis=1,
                errors="ignore",
            )
            ...
            print(f"Erro criar/processar DF: {e}")
            traceback.print_exc()
            return None
    ...
    def _calculate_aggregated_test_metrics(self) -> dict:
        ...
        for k in keys:
            ...
            attempted = [d for d in dets if d["status"] != "No Code"]
            times = [
                d["execution_time_ms"]
                for d in dets
    ...
    def _save_environment_info(self, ts: str):
        ...
        try:
            ...
            print(f"Erro salvar info amb: {e}")
    ...
    def _save_full_results_json(self, ts: str):
        ...
        try:
            ...
            with open(p, "w", encoding="utf-8") as f:
                json.dump(self.results, f, indent=2, ensure_ascii=False, cls=NpEnc)
    ...
    def _save_detailed_tests_csv(self, ts: str):
        ...
        if self.detailed_test_results:
            ...
            print("- Nenhum detalhe teste p/ salvar CSV.")
    ...
    def _save_summary_csv(self, df: pd.DataFrame, ts: str):
        ...
        if df is None:
            print("Erro: DF resumo é None. Pulando CSV.")
            return
        try:
            ...
            print(f"Erro salvar resumo CSV: {e}")
    ...
    def _calculate_statistics(self, df: pd.DataFrame) -> pd.DataFrame | None:
        ...
        try:
            ...
            stats_data = {"Metric": [m[0] for m in metrics_def]}
            for ai in ais:
                df_ai = df[df["ai"] == ai]
                col_data = []
    ...
    def _save_statistics_csv(self, stats_df: pd.DataFrame, ts: str):
        ...
        if stats_df is None:
            print("Erro: DF Stats é None. Pulando CSV stats.")
            return
        ...
        try:
            ...
            print(f"Erro salvar stats CSV: {e}")
                ...
                if not pd.isna(raw_value):
                    try:  # Adiciona try para robustez
                        if "Flake8" in metric_name:
                            ...
                            fmt_value = f"{raw_value:.1f}"  # LoC
                    except (ValueError, TypeError):
                        fmt_value = str(raw_value)  # Fallback
    ...
    def generate_report(self):
        ...
        if not self.results:
            print("Não há resultados p/ relatório.")
            return
        print("\n--- Gerando Relatórios ---")
        ...
        if df is None:
            print("Falha preparar DF. Abortando.")
            return
        ...
        # --- DEBUG: Inspecionar o DataFrame stats_df ---
        print("\n--- DEBUG: Verificando stats_df após _calculate_statistics ---")
        if stats is not None:
            print("stats_df.info():")
            stats.info()
            print("\nstats_df.head():")
            print(stats.head())
            print(f"\nstats_df tem linhas duplicadas? {stats.duplicated().any()}")
            print(f"Número de linhas em stats_df: {len(stats)}")
            # Verifica se a coluna 'Metric' tem duplicados, o que seria esperado se houvesse blocos repetidos
            print(
                f"stats_df tem 'Metric' duplicados? {stats.duplicated(subset=['Metric']).any()}"
            )
            print("Contagem de valores na coluna 'Metric':")
            print(stats["Metric"].value_counts())
        else:
            print("stats_df é None.")
        print("--- FIM DEBUG ---\n")
        # --- FIM DEBUG ---
    ...
    def _plot_bar_charts(self, df_plot, ai_list, cmap, challenge_order, ts, out_dir):
        ...
        metrics_def = {
            ...
            "4_f8": ("Flake8 Iss", "Iss", "flake8_issues"),
        ...
        for key, (title, ylabel, col_name) in metrics_def.items():
            ...
            try:
                ...
                path = os.path.join(out_dir, f"{key}_{ts}.png")
                plt.savefig(path)
                plt.close()
            except Exception as e:
                print(f"Erro bar '{title}': {e}")
                plt.close()

    # Adicione esta função dentro da classe AIBenchmark
    ...
    def _plot_lru_times_chart(self, detailed_df, ai_list, cmap, ts, out_dir):
        ...
        # 2. Calcular a média do tempo por IA (cada linha já representa uma execução agregada do LRU)
        #    Se você tiver múltiplas execuções por AI (o que não parece ser o caso aqui,
        #    pois há 1 linha detalhada por AI/LRU), você agregaria aqui.
        #    Neste caso, podemos pegar diretamente os valores por AI.
        avg_times_per_ai = lru_data.set_index("ai")["execution_time_ms"]

        # Reordena para garantir consistência com outras legendas
        avg_times_per_ai = avg_times_per_ai.reindex(ai_list).dropna()

        if avg_times_per_ai.empty:
            print(f"  Skip LRU Times chart: Sem dados válidos após reindexar por IA.")
            return
        ...
        try:
            ...
            )  # Rotação 0 pode ser melhor aqui
            ax.tick_params(axis="y", labelsize=9)
            ...
            for container in ax.containers:
                ax.bar_label(
                    container, fmt="%.2f", label_type="edge", padding=3, fontsize=8
                )

            # Ajusta limite Y se necessário (começa em 0)
            ax.set_ylim(bottom=0, top=ax.get_ylim()[1] * 1.1)  # Adiciona 10% de espaço

            plt.tight_layout()
            # Usar um número/prefixo consistente para ordenação de arquivos
            plot_prefix = "8_lru_exec_time"  # Exemplo de prefixo
    ...
    def _plot_box_plots(self, df_plot, ai_list, cmap, ts, out_dir):
        ...
        # DEBUG: Verificar o DataFrame que chega na função
        print("\n--- DEBUG: df_plot recebido em _plot_box_plots ---")
        try:
            df_plot.info()  # Mostra tipos de dados e contagem não-nula
            print(
                df_plot[
                    [
                        "ai",
            ...
            )  # Mostra algumas linhas das colunas relevantes
        except Exception as e:
            print(f"Erro ao imprimir debug info do df_plot: {e}")
        print("--- FIM DEBUG ---\n")
        ...
        if (
            ...
            metrics_def["err"] = {
                "t": "Distribuição Erros/Falhas Exec",
                "y": "Num",
                "v": (lambda d, c: d[c].notna()),
            }
        ...
        for key, config in metrics_def.items():
            ...
            # DEBUG: Mostrar dados ANTES do filtro
            print(f"  Dados ANTES do filtro para '{col_name}':")
            print(
                temp_df[col_name].value_counts(dropna=False).head()
            )  # Mostra contagem dos valores mais comuns (inclui NaN)
            print(
                f"  Total não-nulo antes: {temp_df[col_name].notna().sum()}, Total linhas: {len(temp_df)}"
            )
            ...
            if filter_func:
                try:
                    temp_df = temp_df[filter_func(temp_df, key)]
                except Exception as e:
                    print(f" Aviso box cond '{key}': {e}.")
            if temp_df.empty or temp_df[key].isnull().all():
                print(f"  Skip box '{config['t']}': Sem dados.")
                continue

            # DEBUG: Mostrar dados DEPOIS do filtro
            print(f"  Dados DEPOIS do filtro para '{col_name}':")
            if not temp_df.empty:
                print(temp_df[col_name].value_counts(dropna=False).head())
                print(
                    f"  Total não-nulo depois: {temp_df[col_name].notna().sum()}, Total linhas: {len(temp_df)}"
                )
            else:
            ...
            if temp_df.empty or temp_df[col_name].isnull().all():
                print(
                    f"  >>> Skip box '{config['t']}': Sem dados válidos após filtro."
                )  # DEBUG: Mensagem mais clara
                print("--- FIM DEBUG ---")
                continue
            ...
            if not plot_data:
                print(f"  Skip box '{config['t']}': Sem dados por IA.")
                continue
            ...
            try:
                ...
                for patch, color in zip(bp["boxes"], colors):
                    ...
                    patch.set_alpha(0.6)
                plt.tight_layout()
                fname = f"{plot_counter}_boxplot_{key}_{ts}.png"
                p = os.path.join(out_dir, fname)
                plt.savefig(p)
                plt.close(fig)
            except Exception as e:
                print(f"  Erro box plot '{config['t']}': {e}")
                plt.close()  # Fecha figura em caso de erro
        print("Geração Box Plots concluída.")

    # Substitua a função inteira por esta versão com debug aprimorado
    def _plot_box_plots(self, df_plot, ai_list, cmap, ts, out_dir):
        """Gera box plots."""
        print("Gerando Box Plots...")
        # DEBUG: Verificar o DataFrame que chega na função
        ...
        try:
            ...
            print(f"Erro ao imprimir debug info do df_plot: {e}")
        print("--- FIM DEBUG: df_plot recebido ---\n")

        # Construção do dicionário de métricas
        metrics_def = {
            "api_execution_time": {
                "t": "Distribuição API Time",
                "y": "Tempo(s)",
                "v": None,
            },
            "lines_of_code": {
        ...
        # DEBUG: Verificar o dicionário metrics_def ANTES do loop
        print(f"\n--- DEBUG: metrics_def ANTES do loop ---")
        print(metrics_def.keys())  # Mostra as chaves que serão iteradas
        print(f"--- FIM DEBUG: metrics_def ANTES do loop ---\n")

        plot_counter = 7  # Contador nome ficheiro

        if not metrics_def:
            print(">>> ERRO/AVISO: metrics_def está vazio! Pulando todos os box plots.")
        else:
            # Iterar usando os nomes das colunas como chave
            for col_name, config in metrics_def.items():
                plot_counter += 1
                # DEBUG: Print no INÍCIO da iteração do loop
                print(
                    f"\n--- DEBUG: Iniciando iteração Box Plot para: {col_name} ({config['t']}) ---"
                )
                ...
                if col_name not in df_plot.columns:
                    ...
                    print("--- FIM DEBUG: Iteração ---")  # DEBUG: Fim da iteração
                    continue
                ...
                try:
                    ...
                    print(f"    Erro ao mostrar dados antes: {e}")
                ...
                if filter_func:
                    try:
                        ...
                        print(
                            f"  Filtro aplicado para '{col_name}'. Linhas antes: {original_rows}, Linhas depois: {rows_after}"
                        )
                    except Exception as e:
                        print(
                ...
                if not temp_df.empty:
                    try:
                        ...
                        print(f"    Erro ao mostrar dados depois: {e}")
                else:
                    print("  DataFrame vazio após filtro.")
                ...
                # Coleta dados por AI (após o filtro)
                for ai in ai_list:
                    ai_data = temp_df[temp_df["ai"] == ai][col_name].dropna()
                    if not ai_data.empty:
                        if ai_data.nunique() > 1:
                            plot_data.append(ai_data.values)
                            plot_labels.append(ai)
                            print(
                                f"    Dados para IA '{ai}' (coluna '{col_name}'): {len(ai_data)} pontos válidos."
                            )
                        else:
                            print(
                                f"    AVISO: Dados para IA '{ai}' (coluna '{col_name}') têm apenas 1 valor único ({ai_data.iloc[0]}). Incluindo."
                            )
                            plot_data.append(ai_data.values)
                            plot_labels.append(ai)
                ...
                if not plot_data:
                    print(
                        f"  >>> Skip box '{config['t']}': Sem dados válidos por IA após filtro."
                    )
                    print("--- FIM DEBUG: Iteração ---")  # DEBUG: Fim da iteração
                    continue
                ...
                try:
                    ...
                    for patch, color in zip(bp["boxes"], colors):
                        ...
                        patch.set_alpha(0.6)
                    plt.tight_layout()
                    fname = f"{plot_counter}_boxplot_{col_name}_{ts}.png"
                    p = os.path.join(out_dir, fname)
                    plt.savefig(p)
                    plt.close(fig)
                ...
                # DEBUG: Print no FIM da iteração do loop
                print("--- FIM DEBUG: Iteração ---")
    ...
    def create_visualizations(self, df, ts, out_dir):
        ...
        if df is None or df.empty:
            print("AVISO: DF vazio p/ gráficos. Pulando.")
            return
        plt.style.use("ggplot")
        ais = sorted(df["ai"].unique())
        n = len(ais)
        if n == 0:
            print("AVISO: Nenhuma AI encontrada nos dados para plotar.")
            return

        # Define mapa de cores
        colors = (
            plt.cm.tab10(np.linspace(0, 1, n))
            if n <= 10
            else plt.cm.viridis(np.linspace(0, 1, n))
        )
        ...
        req = [
            ...
            c for c in ["avg_exec_success_rate", "avg_exec_time_ms"] if c in df.columns
        ]
        ...
        if "difficulty" in df.columns:
            ...
            try:
                ...
                print(f"AVISO: Erro ordenar dificuldade ({e_cat}).")

        # --- NOVO: Prepara DataFrame Detalhado para Plot LRU ---
        detailed_df = None
        if self.detailed_test_results:
            try:
                detailed_df = pd.DataFrame(self.detailed_test_results)
                # Converte tempo para numérico, tratando erros/nulos
        ...
        # --- NOVO: Chama a função de plot LRU ---
