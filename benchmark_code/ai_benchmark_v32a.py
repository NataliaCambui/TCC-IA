...
class AIBenchmark:
    ...
    def extract_code(self, content: str) -> str:
        """Extrai código Python de uma resposta de modelo.
        - Prioriza blocos ```python ... ``` ou ``` ... ```.
        - Caso contrário, tenta heurística por indentação/def/class e para corretamente quando o bloco termina.
        """
        if not content:
            return ""
        ...
        # 3) Heurística por indentação/def/class
        lines_content = content.splitlines()
        code_lines = []
        in_block = False
        for line in lines_content:
            stripped = line.strip()
            if not in_block and (line.startswith((" ", "\t")) or stripped.startswith(("def ", "class "))):
                in_block = True
            if in_block:
                if (not line.startswith((" ", "\t"))) and (not stripped.startswith(("def ", "class "))):
                    break
        ...
        # 4) Fallback
        looks_like_code = any(tok in content for tok in ("def ", "class ", ":\n", " for ", " in "))
        return content.strip() if looks_like_code else ""
    ...
    def _lru_worker(
        ...
        try:
            ...
            # Prefer a class explicitly named 'LRUCache' when available
            LruClass = local_namespace.get("LRUCache")
            class_name = None
            if not LruClass:
                for name, obj in local_namespace.items():
                    if isinstance(obj, type) and name not in ("OrderedDict",):
                        LruClass = obj
                        class_name = name
                        break
            else:
