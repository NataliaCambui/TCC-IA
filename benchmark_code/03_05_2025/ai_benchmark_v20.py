...
import black as black # Import necessário para formatação de código
...
class AIBenchmark:
    ...
    def __init__(self):
        ...
        self.code_challenges = [ # Lista de desafios
        ...
        try: # Tenta importlib.metadata primeiro (Python 3.8+)
            ...
            if pkg_resources: # Verifica se pkg_resources foi importado com sucesso
                for lib in libs:
                    ...
                    except Exception as e: lib_versions[lib] = f"Error ({type(e).__name__})" # Simplificado
        ...
        try:
            ...
            try:
                content=response.text
                 ...
                 # Bloco de feedback simplificado (removido try/except interno)
                 if hasattr(response, 'prompt_feedback'):
                      print(f"  Feedback: {response.prompt_feedback}")
                 if hasattr(response, 'candidates') and response.candidates:
                      print(f"  Reason: {response.candidates[0].finish_reason}")
        # Excepts externos mantidos e indentados corretamente
        except google.api_core.exceptions.ResourceExhausted as e:
            print(f"Erro Gemini (Rate Limit): {e}")
            if execution_time==0: execution_time=time.time()-start_time # Garante tempo
        except Exception as e:
            print(f"Erro inesperado Gemini: {type(e).__name__}-{e}")
            # --- CORREÇÃO v19: 'if' movido para nova linha ---
            if execution_time==0:
                execution_time=time.time()-start_time # Garante tempo
    ...
    # --- Métodos Execução Local LRU Cache (Correção __name__ v19) ---
    ...
    def _lru_worker(self, code_string: str, test_sequence: list, result_queue: multiprocessing.Queue):
        ...
        try:
            ...
            safe_builtins = {
                ...
                # Adiciona __name__ aqui também para garantir
                '__name__': '__exec_sandbox__'
            }
            safe_globals = {
                "__builtins__": safe_builtins,
                "OrderedDict": OrderedDict,
                 # Garante que __name__ esteja no globals também
                "__name__": "__exec_sandbox__"
            }
            ...
            for op_data in test_sequence:
                ...
                # if not step_res["ok"]: break # Opcional
    ...
    def _detect_function_name(self, code: str, challenge: dict) -> str | None:
        ...
        if sig_match:
            ...
            # --- CORREÇÃO v19: Tenta busca simples primeiro ---
            simple_def = f"def {expected_name}("
            simple_class = f"class {expected_name}:"
            simple_class_paren = f"class {expected_name}("

            # Procura por definições no início de linhas (permitindo indentação)
            # Usando re.escape para nomes que podem ter caracteres especiais (improvável aqui)
            ...
            # --- CORREÇÃO v19: Adiciona verificação 'in' antes do fallback regex ---
            # Verifica se o nome esperado existe em qualquer lugar como palavra separada
            if re.search(rf"\b{re.escape(expected_name)}\b", code):
                 print(f"  AVISO: Nome esperado '{expected_name}' encontrado no código, mas não como definição padrão. Usando-o.")
                 return expected_name

            # Fallback para regex menos restrito (procura em qualquer linha)
            pattern = rf"\s*(?:def|class)\s+{re.escape(expected_name)}\b"
            exact_match = re.search(pattern, code, re.MULTILINE)
            if exact_match:
                 return expected_name

        # Fallback final: primeira definição encontrada
    ...
    def _create_csv_header(self) -> str:
        ...
        # Simplificado para usar loop normal
        for k,v in self.environment_info.items():
            if isinstance(v,dict):
                 hdr.append(f"# {k}:")
                 for sk, sv in v.items(): hdr.append(f"#   {sk}: {sv}")
             ...
             for ai in ais:
                 ...
                 for _,col,agg in m:
                     ...
                     if col in df_ai.columns:
                        series = df_ai[col]
                        if agg=="mean": v=series.mean()
                        elif agg=="sum": v=series.sum()
                        elif agg=="mean_valid": d=series[series.notna()&(series!=-1)]; v=d.mean() if not d.empty else np.nan
                        elif agg=="mean_percentage": v=series.mean()*100 if series.notna().any() else 0.0 # Evita NaN%
                        elif agg=="first_non_empty": errs=series[series.fillna('').astype(str)!='']; v=errs.iloc[0] if not errs.empty else ""
                     c_data.append(v)
                 data[ai]=c_data
    ...
    def _display_summary_table(self, stats_df: pd.DataFrame):
        ...
        disp=stats_df.copy()
        # Formata colunas existentes de forma mais segura
        for col in disp.columns:
            ...
            for i in disp.index:
                ...
                if not pd.isna(v):
                    ...
                    except (ValueError, TypeError): fmt = str(v) # Fallback
                fmt_col.append(fmt)
            disp[col]=fmt_col
        # Imprime tabela
    ...
    def _plot_bar_charts(self, df_plot, ai_list, cmap, challenge_order, ts, out_dir):
        ...
        # Simplificado para usar loops padrão
        metrics = {
        ...
        for k,(t,y,c) in metrics.items():
            ...
            elif c=='avg_exec_success_rate': data.loc[:,c]=data[c]*100
            ...
            try:
                ...
                n_ai=len(ai_list)
                ax=piv.plot(kind='bar',figsize=(max(12,n_ai*2),7), color=[cmap.get(ai,'grey') for ai in piv.columns],width=0.8)
                ...
                if c in ['completeness','avg_exec_success_rate']: plt.ylim(0,105)
                    ...
                    mv=piv.max(skipna=True).max(skipna=True); plt.ylim(bottom=0,top=max(1,mv*1.1) if not pd.isna(mv) else 1)
    ...
    def _plot_box_plots(self, df_plot, ai_list, cmap, ts, out_dir):
        ...
        for k,cfg in metrics.items():
            ...
            try:
                ...
                bp=plt.boxplot(data,tick_labels=lbls,patch_artist=True,showfliers=True) # Usa tick_labels
