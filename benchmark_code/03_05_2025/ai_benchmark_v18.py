...
try:
    ...
    import pkg_resources # type: ignore # Ignora aviso Pylance sobre pkg_resources
...
class AIBenchmark:
    ...
    """
    # --- __init__ e Configuração (sem alterações de indentação) ---
    def __init__(self):
    ...
    def _get_environment_info(self):
        ...
        try:
            import importlib.metadata as importlib_metadata # Tenta import moderno
            for lib in libs:
                try:
    ...
    # --- Funções de consulta API (sem alterações de indentação) ---
    def query_claude(self, prompt: str) -> dict:
        ...
        try:
            ...
            if e.response is not None:
                try:
                    err_msg += f" Status:{e.response.status_code} Body:{e.response.json()}"
                except Exception:
    ...
    def query_gemini(self, prompt: str) -> dict:
        ...
        try:
            model=genai.GenerativeModel(self.gemini_model); safety=[{"category":c,"threshold":"BLOCK_NONE"} for c in ["HARM_CATEGORY_HARASSMENT","HARM_CATEGORY_HATE_SPEECH","HARM_CATEGORY_SEXUALLY_EXPLICIT","HARM_CATEGORY_DANGEROUS_CONTENT"]]
            response=model.generate_content(prompt,safety_settings=safety); execution_time=time.time()-start_time
            try: content=response.text
            except ValueError: print(f"Resposta Gemini bloqueada?"); 
            try: print(f" Feedbk:{response.prompt_feedback}"); 
            if hasattr(response,'candidates') and response.candidates: print(f" Reason:{response.candidates[0].finish_reason}"); 
        except Exception as fe: print(f" Err feedbk:{fe}")
    ...
    # --- Funções de processamento de código (sem alterações de indentação) ---
    ...
    # --- Métodos Execução Local LRU Cache (Indentação Verificada) ---
    ...
    def _lru_worker(self, code_string: str, test_sequence: list, result_queue: multiprocessing.Queue):
        ...
        try:
            ...
            safe_builtins = {
                ...
                '__name__': '__exec_sandbox__' # Correção v17
            }
            safe_globals = {
            ...
            if not LruClass:
                LruClass = safe_globals.get(class_name) # Tenta globals como fallback
                if not LruClass:
    ...
    def evaluate_solution(self, ai_name: str, challenge: dict, response: dict) -> dict:
        ...
        if can_exec and challenge["name"] != "LRU Cache":
            # Passa dict challenge para ter acesso à assinatura
        ...
        for i, tc in enumerate(test_cases):
            ...
            if exec_res.get("success") is True:
                ...
                try:
                    ...
                    t_det["status"] = "Pass" if eq else "Fail"
                except Exception as e:
                    t_det["status"] = "Fail"
                    t_det["error_message"] = f"Comparison error: {e}"
    ...
    # --- run_benchmark (Indentação Verificada) ---
    def run_benchmark(self):
        ...
        ch_details = { "Fibonacci Sequence": "Téc: Iterativa/Recursiva. Fin: Algoritmos básicos, sequências, casos base.", "Binary Search": "Téc: Dividir/Conquistar. Fin: Busca binária, índices, condições parada.", "Merge Sort": "Téc: Dividir/Conquistar (recursão/merge). Fin: Ordenação recursiva, divisão, intercalação.", "Count Islands in Grid": "Téc: Travessia Grafo (DFS/BFS). Fin: Modelar grafo, DFS/BFS, matriz 2D, visitados.", "Palindrome Check": "Téc: Manipulação Strings. Fin: Processar strings, ignorar caracteres/case, simetria.", "Valid Parentheses": "Téc: Pilha (Stack). Fin: Rastrear/Validar ordem parênteses.", "LRU Cache": "Téc: Estrutura Dados Custom (dict/OrderedDict). Fin: Estruturas complexas, capacidade, get/put O(1), política despejo." }
        ...
        for chal in self.code_challenges:
            ...
            for ai in enabled_ais:
                ...
                # Avalia e guarda resultado agregado e detalhado
                result=self.evaluate_solution(ai, chal, resp_data)
                self.results.append(result)

                # Gera linha de resumo (usa a lista principal de detalhes agora preenchida)
                ...
                elif chal['name']=="LRU Cache": status=det[0]['status'] if det else "Error" # Usa status do único detalhe LRU
            ...
            # Imprime todas as linhas de resumo
    ...
    # --- Métodos de Geração de Relatório (Simplificados e Indentação Verificada) ---
    ...
    def _calculate_aggregated_test_metrics(self) -> dict:
        ...
        for key in all_keys:
            ...
            attempted_runs = [d for d in details if d['status'] != 'No Code']
            # Inclui tempos de execução mesmo de falhas para avg_exec_time_ms
            ...
            mean_time = np.mean(valid_times) if valid_times else np.nan # Usa np.nan se não houver tempos
    ...
    def _normalize_dataframe_types(self, df: pd.DataFrame):
        ...
        for col in num_cols:
            if col in df.columns:
                ...
                fill_val = 0.0; dtype = None
                if col=='flake8_issues': fill_val=-1; dtype=int
                elif col in ['num_execution_errors','total_tests_passed','total_tests_attempted']: fill_val=0; dtype=int
                elif col=='avg_exec_time_ms': fill_val=-1.0 # Usa -1 para NaN/Não aplicável
                elif col=='avg_exec_success_rate': fill_val=0.0

                df.loc[:, col] = df[col].fillna(fill_val)
                if dtype: # Aplica tipo inteiro apenas onde especificado
                     try: df.loc[:, col] = df[col].astype(dtype)
                     except ValueError as e: print(f"Aviso: Não foi possível converter coluna '{col}' para {dtype}. Erro: {e}")
    ...
    def _save_detailed_tests_csv(self, ts: str):
        ...
        if self.detailed_test_results:
            try:
                ...
                cols = [ # Ordem e colunas desejadas
                    'ai','challenge','difficulty','test_case_index','status',
                    'execution_time_ms','error_message','stdout',
                    'input_args_str','expected_output_str','actual_output_str'
                ]
                # Garante que todas as colunas existem, preenchendo com NA se faltar
                for c in cols:
                 ...
                 for _, col_key, agg_type in metrics_def:
                     ...
                     if col_key in df_ai.columns:
                         ...
                         if agg_type == "mean": value = series.mean()
                             ...
                             value = valid_data.mean() if not valid_data.empty else np.nan
    ...
    def _display_summary_table(self, stats_df: pd.DataFrame):
        ...
        # Itera pelas colunas de IA para formatar
        for col_name in disp_df.columns:
            ...
            for index, row in disp_df.iterrows():
                ...
                if not pd.isna(raw_value):
                    if "Flake8" in metric_name:
                        ...
                        s_val = str(raw_value)
                        fmt_value = s_val[:60] + ('...' if len(s_val) > 60 else '')
                    else: # Default para outros floats (LoC)
                         fmt_value = f"{raw_value:.1f}"

                formatted_values.append(fmt_value)
            # Atualiza a coluna no DataFrame de display
            disp_df[col_name] = formatted_values

        # Tenta imprimir com tabulate
        try:
            from tabulate import tabulate
            print(tabulate(disp_df, headers='keys', tablefmt='grid', showindex=False))
        except ImportError:
            print("(Instale 'tabulate' p/ tabela bonita)")
            print(disp_df.to_string(index=False)) # Fallback
        except Exception as e:
    ...
    # --- Funções de Plotagem (Simplificadas) ---
    ...
    def _plot_bar_charts(self, df_plot, ai_list, cmap, challenge_order, ts, out_dir):
        ...
        for k,(t,y,c) in metrics.items():
            ...
            try:
                plt.figure()
                piv=data.pivot_table(index='challenge',columns='ai',values=c,aggfunc='mean')
                piv=piv.reindex(challenge_order,axis=0).reindex(ai_list,axis=1)
                fill=0.0 if c not in ['flake8_issues','avg_exec_time_ms','num_execution_errors'] else np.nan
                piv.fillna(fill,inplace=True)
                if piv.isnull().all().all(): print(f"Skip bar '{t}': NaN."); plt.close(); continue

                n_ai=len(ai_list)
                ax=piv.plot(kind='bar',figsize=(max(12,n_ai*2),7),
                            color=[cmap.get(ai,'grey') for ai in piv.columns],width=0.8)
                plt.title(f'{t} por Desafio'); plt.ylabel(y); plt.xlabel('Desafio')
                ...
                elif c in ['flake8_issues','num_execution_errors'] and not piv.isnull().all().all():
                    mv=piv.max(skipna=True).max(skipna=True)
                    plt.ylim(bottom=0,top=max(1,mv*1.1) if not pd.isna(mv) else 1)
                elif c=='avg_exec_time_ms' and not piv.isnull().all().all():
                    plt.ylim(bottom=0)
                plt.legend(title='IA',bbox_to_anchor=(1.02,1),loc='upper left')
                plt.tight_layout(rect=[0,0,0.88,1])
                p=os.path.join(out_dir,f"{k}_{ts}.png"); plt.savefig(p); plt.close()
    ...
    def _plot_box_plots(self, df_plot, ai_list, cmap, ts, out_dir):
        ...
        for k,cfg in metrics.items():
            ...
            try:
                plt.figure(figsize=(max(8,len(lbls)*1.2),6))
                bp=plt.boxplot(data,tick_labels=lbls,patch_artist=True,showfliers=True)
                plt.title(cfg["t"],fontsize=14); plt.ylabel(cfg["y"],fontsize=10); plt.xlabel("IA",fontsize=10)
                ...
                plt.tight_layout()
                fname=f"{cnt}_boxplot_{k}_{ts}.png"; p=os.path.join(out_dir,fname); plt.savefig(p); plt.close()
    ...
    def _plot_radar_chart(self, df, ai_list, cmap, ts, out_dir):
        ...
        try:
            ...
            cats=['Veloc API(Norm)','Conc LoC(Norm)','Completude(%)']; N=len(cats)
            ang=np.linspace(0, 2 * np.pi, N, endpoint=False).tolist(); ang+=ang[:1] # Fecha o círculo
            fig=plt.figure(figsize=(9,9)); ax=fig.add_subplot(111,polar=True)
            for ai in ai_list:
                vals=[t_norm.get(ai,0),l_norm.get(ai,0),c_norm.get(ai,0)]
                pv=vals+[vals[0]] # Fecha o polígono
    ...
    def create_visualizations(self, df, ts, out_dir):
        ...
        if df.empty or not all(c in df.columns for c in req):
            print("AVISO: Faltando cols p/ gráficos. Pulando.")
            return
        ...
        self._plot_radar_chart(df,ais,cmap,ts,out_dir)
