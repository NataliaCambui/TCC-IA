...
class AIBenchmark:
    ...
    def _get_environment_info(self):
        ...
        info = {
            "Timestamp": datetime.now().isoformat(), "OS": f"{platform.system()} {platform.release()}",
            "Python Version": sys.version.split()[0],
            "Models": {"Claude": self.claude_model if self.claude_api_key else "N/A", "ChatGPT": self.openai_model if self.openai_api_key else "N/A", "Gemini": self.gemini_model if self.gemini_api_key else "N/A",},
            "Executor Service URL": self.executor_url, "Libraries": {}
        }
        libs_to_check = ['pandas', 'numpy', 'matplotlib', 'requests', 'python-dotenv', 'google-generativeai', 'google-api-core']
        try:
            for lib in libs_to_check:
             ...
             for lib in libs_to_check:
                try: info["Libraries"][lib] = pkg_resources.get_distribution(lib).version
                     ...
                     # Pylance reportou erro aqui [Ln ~129] - Verifique a sintaxe e caracteres ocultos nesta linha e na anterior no seu arquivo.
                     info["Libraries"][lib] = "Not Found"
                except Exception:
                    # Pylance reportou erro aqui [Ln ~130] - Verifique a indentação e a sintaxe desta linha e da anterior.
                    info["Libraries"][lib] = "Error getting version"
        return info

    def _check_flake8(self):
        try: subprocess.run(['flake8', '--version'], check=True, capture_output=True, timeout=5); print("Verificação Flake8: OK"); self.flake8_available = True
        except Exception: print("AVISO: Comando 'flake8' não encontrado/executável. Análise de qualidade será pulada."); self.flake8_available = False
    ...
    def query_claude(self, prompt):
        ...
        try:
            ...
            execution_time = time.time() - start_time; error_message = f"Erro consulta Claude (RequestException): {e}"
            if e.response is not None:
                error_message += f" | Status: {e.response.status_code}"
                try:
                    # Pylance reportou erro aqui [Ln ~168] - Verifique sintaxe do f-string, chaves, aspas, etc.
                    error_message += f" | Body: {e.response.json()}"
                except json.JSONDecodeError:
                     # Pylance reportou erro aqui [Ln ~170] - Verifique sintaxe do f-string, chaves, aspas, etc.
                    error_message += f" | Body: {e.response.text}"
            print(error_message); return {"content": "", "execution_time": execution_time}
        except Exception as e: execution_time = time.time() - start_time; print(f"Erro inesperado consulta Claude: {type(e).__name__} - {e}"); return {"content": "", "execution_time": execution_time}

    # Funções query_chatgpt, query_gemini, extract_code, count_code_lines, measure_response_completeness, analyze_code_quality_flake8, evaluate_solution, _call_executor_service
    # (Mantidas iguais)
    def query_chatgpt(self, prompt):
        ...
        try:
            ...
            execution_time = time.time() - start_time; error_message = f"Erro consulta ChatGPT (RequestException): {e}"
            if e.response is not None:
                error_message += f" | Status: {e.response.status_code}"
                try: error_details = e.response.json(); error_msg = error_details.get('error', {}).get('message', ''); error_message += f" | API Error: {error_msg}" if error_msg else f" | Body: {error_details}"
                except json.JSONDecodeError: error_message += f" | Body: {e.response.text}"
            print(error_message); return {"content": "", "execution_time": execution_time}
        except Exception as e:
            execution_time = time.time() - start_time; print(f"Erro inesperado consulta ChatGPT: {type(e).__name__} - {e}"); return {"content": "", "execution_time": execution_time}
    ...
    def query_gemini(self, prompt):
        ...
        try:
            ...
            execution_time = time.time() - start_time; print(f"Erro consulta Gemini (ResourceExhausted - Rate Limit): {e}\n                    >> Atingido o limite de requisições da API Gemini. Tente novamente mais tarde."); return {"content": "", "execution_time": execution_time}
        except Exception as e:
            execution_time = time.time() - start_time; print(f"Erro inesperado consulta Gemini: {type(e).__name__} - {e}"); return {"content": "", "execution_time": execution_time}
    ...
    def analyze_code_quality_flake8(self, code_string):
        ...
        try:
            ...
            if result.returncode in [0, 1] and result.stdout:
                ...
                except (IndexError, ValueError): return len(result.stdout.strip().splitlines()) # Fallback
    ...
    def evaluate_solution(self, ai_name, challenge, response):
        ...
        execution_results_list = []; processed_test_details = []
        num_execution_errors = 0; first_execution_error = ""
        ...
        if code and actual_function_name and challenge["name"] != "LRU Cache":
            ...
            if not test_cases: print(f"   Aviso: Nenhum caso de teste definido para '{challenge['name']}'. Pulando execução.")
                ...
                for i, test_case in enumerate(test_cases):
                    ...
                    test_detail = {"ai": ai_name, "challenge": challenge["name"], "difficulty": challenge["difficulty"],"test_case_index": i, "input_args_str": json.dumps(input_args), "expected_output_str": json.dumps(expected_output),"actual_output_str": None, "status": "Error", "execution_time_ms": exec_result.get("execution_time_ms", -1),"error_message": None, "stdout": exec_result.get("stdout")}
                    if exec_result.get("success") is True:
                        actual_output = exec_result.get("output"); test_detail["actual_output_str"] = json.dumps(actual_output)
                        try:
                            if np.array_equal(np.array(actual_output, dtype=object), np.array(expected_output, dtype=object)): test_detail["status"] = "Pass"
                            else: test_detail["status"] = "Fail"
                        except Exception as compare_err: test_detail["status"] = "Fail (Comparison Error)"; test_detail["error_message"] = f"Error comparing outputs: {compare_err}"
                    elif exec_result.get("success") is False:
                        test_detail["status"] = "Error"; test_detail["error_message"] = exec_result.get("error", "Unknown execution error"); num_execution_errors += 1
                        if not first_execution_error and test_detail["error_message"]: first_execution_error = test_detail["error_message"]
                    else:
                         test_detail["status"] = "Unknown"; test_detail["error_message"] = exec_result.get("error", "Executor did not return success status"); num_execution_errors += 1
                         if not first_execution_error and test_detail["error_message"]: first_execution_error = test_detail["error_message"]
            ...
            execution_results_list.append({ "success": False, "output": None, "stdout": None, "error": error_msg, "execution_time_ms": -1, "test_case_index": 0, "input_args": None, "expected_output": None })
            num_execution_errors = 1; first_execution_error = error_msg
            processed_test_details.append({"ai": ai_name, "challenge": challenge["name"], "difficulty": challenge["difficulty"], "test_case_index": 0,"input_args_str": "N/A (LRU)", "expected_output_str": "N/A (LRU)", "actual_output_str": None,"status": "Not Executed", "execution_time_ms": -1, "error_message": error_msg, "stdout": None})
        self.detailed_test_results.extend(processed_test_details)
        return {"ai": ai_name, "challenge": challenge["name"], "difficulty": challenge["difficulty"], "api_execution_time": api_execution_time,"lines_of_code": lines_of_code, "completeness": completeness, "flake8_issues": flake8_issues,"num_execution_errors": num_execution_errors, "first_execution_error": first_execution_error,"code": code, "full_response": content, "execution_results_raw": execution_results_list}
    ...
    def _call_executor_service(self, code_string, function_name, input_args):
        ...
        try:
            ...
            else: print(f"   Erro: Chamada ao executor falhou. Status: {response.status_code}, Resposta: {response.text[:200]}..."); default_error_result["error"] = f"Executor service returned status {response.status_code}: {response.text[:100]}"; return default_error_result
    ...
    def run_benchmark(self):
        ...
        print(f"  Gemini Model : {self.gemini_model if can_run_gemini else 'N/A (No Key)'}")
        print("==========================================")
        ...
        for challenge in self.code_challenges:
            ...
            for ai_name in enabled_ais:
                ...
                tests_attempted = len(related_details); tests_passed = len([d for d in related_details if d['status'] == 'Pass'])
                summary_line = (f"  {ai_name:<10}: Tests: {tests_passed}/{tests_attempted} passed ({tests_error} errors). Flake8: {result['flake8_issues'] if result['flake8_issues'] != -1 else 'N/A'}. API Time: {result['api_execution_time']:.2f}s.")
    ...
    def generate_report(self):
        ...
        for key, value in self.environment_info.items():
            if isinstance(value, dict): print(f"  {key}:"); [print(f"      {sk}: {sv}") for sk, sv in value.items()]
            else: print(f"  {key}: {value}")
        print("-------------------------------------------")
        try:
            df = pd.DataFrame(self.results); agg_metrics = {}
            for _, row in df.iterrows():
                    key = (row['ai'], row['challenge']); details = [d for d in self.detailed_test_results if d['ai'] == key[0] and d['challenge'] == key[1]]
                    ...
                    valid_times = [d['execution_time_ms'] for d in details if d.get('execution_time_ms', -1) >= 0 and d['status'] not in ['Unknown', 'Not Executed']]
                    num_attempted = len([d for d in details if d['status'] in ['Pass', 'Fail', 'Error']]); num_passed = len(passed_runs)
                    success_rate = num_passed / num_attempted if num_attempted > 0 else 0.0; mean_time = np.mean(valid_times) if valid_times else np.nan
        ...
        try:
            ...
            env_comments.append("# --- Benchmark Summary ---"); env_header = "\n".join(env_comments) + "\n"
            # A linha abaixo foi corrigida: line_terminator -> lineterminator
        ...
        try:
            ...
            for ai_name in ai_list:
                ...
                for metric_name, col_key, agg_type in stats_metrics:
                    ...
                    elif agg_type == "first_non_empty": first_err = df_ai[df_ai[col_key] != ''][col_key].iloc[0] if not df_ai[df_ai[col_key] != ''].empty else ""; avg_val = first_err
                    else: avg_val = np.nan
                    ai_column_data.append(avg_val)
                stats_data[ai_name] = ai_column_data
            stats = pd.DataFrame(stats_data)
            env_comments_stats = ["# --- Environment Info ---"];
            for key, value in self.environment_info.items():
                    if isinstance(value, dict): env_comments_stats.append(f"# {key}:"); [env_comments_stats.append(f"#   {sk}: {sv}") for sk, sv in value.items()]
                    else: env_comments_stats.append(f"# {key}: {value}")
            env_comments_stats.append("# --- Benchmark Statistics ---"); env_header_stats = "\n".join(env_comments_stats) + "\n"
            # A linha abaixo foi corrigida: line_terminator -> lineterminator
            with open(stats_path, "w", encoding='utf-8', newline='') as f: f.write(env_header_stats); stats.to_csv(f, index=False, encoding='utf-8', float_format='%.2f', lineterminator='\n') # <<< CORRIGIDO >>>
        ...
        # <<< CÓDIGO DE DEPURAÇÃO INSERIDO AQUI >>>
        print("\n===== SUMÁRIO DO BENCHMARK =====")
        if stats is not None:
            stats_display = stats.copy()
            print(f"DEBUG: Colunas do DataFrame de stats: {stats_display.columns.tolist()}") # DEBUG 1
            for col in stats_display.columns:   # Nível 1
                print(f"\nDEBUG: Processando coluna: '{col}'") # DEBUG 2
                if col == 'Metric':
                    print("DEBUG: Pulando coluna 'Metric'.") # DEBUG 3
                    continue

                # Tentativa de garantir a inicialização
                formatted_col = [] # Inicialização Original (Nível 2)
                print(f"DEBUG: -> formatted_col inicializada para '{col}' (ID: {id(formatted_col)})") # DEBUG 4

                if stats_display.empty:
                        print(f"DEBUG: DataFrame de stats está vazio para a coluna '{col}', pulando loop interno.") #DEBUG 5
                        continue # Se não houver linhas/métricas

                print(f"DEBUG: Iniciando loop interno para '{col}' (Índices: {stats_display.index.tolist()})") # DEBUG 6
                # Loop interno (Nível 3)
                for i in stats_display.index:
                    try: # Adiciona try/except para pegar erros no processamento da linha
                        metric_name = stats_display.loc[i, 'Metric']
                        value = stats_display.loc[i, col]
                        print(f"DEBUG:   [Linha {i}, Col '{col}'] Métrica: '{metric_name}', Valor Bruto: {repr(value)}") # DEBUG 7

                        formatted_val = "N/A" # Valor padrão (Nível 4)
                        if not pd.isna(value):
                            # --- Bloco de formatação (Nível 5+) ---
                            ...
                            elif "Example First Error" in metric_name: formatted_val = str(value)[:60] + ('...' if len(str(value)) > 60 else '')
                            else: formatted_val = f"{value:.1f}" # Default para outros números
                            # --- Fim do Bloco de formatação ---
                        else:
                                formatted_val = "N/A" # Garante que é N/A se for NaN

                        print(f"DEBUG:   [Linha {i}, Col '{col}'] Valor Formatado: '{formatted_val}'") # DEBUG 8

                        # Tentativa de append (Nível 3)
                        print(f"DEBUG:   Tentando append em formatted_col (ID: {id(formatted_col)})...") # DEBUG 9
                        formatted_col.append(formatted_val) # Linha ~530 - Onde o erro ocorre
                        print(f"DEBUG:   Append bem-sucedido.") # DEBUG 10

                    except Exception as e_inner:
                            print(f"!!!!! ERRO NO LOOP INTERNO [Linha {i}, Col '{col}']: {e_inner}") # DEBUG 11
                            # Tenta adicionar um valor de erro para não quebrar a estrutura
                            try:
                                    # Verifica se formatted_col existe antes de tentar append
                                    if 'formatted_col' in locals() and isinstance(formatted_col, list):
                                        formatted_col.append("<<ERROR>>")
                                    else:
                                        # Se formatted_col não existir neste escopo interno por algum motivo, inicializa aqui
                                        print(f"!!!!! AVISO: formatted_col não encontrado no escopo interno ao tratar erro. Re-inicializando.")
                                        formatted_col = ["<<ERROR>>"] # Inicializa com o erro
                            except Exception as e_append_err:
                                    print(f"!!!!! ERRO AO TENTAR APPEND APÓS ERRO INTERNO: {e_append_err}")


                print(f"DEBUG: Fim do loop interno para '{col}'. Atualizando DataFrame.") # DEBUG 12
                # Atualiza coluna (Nível 2)
                # Adiciona verificação para garantir que formatted_col é uma lista antes de atribuir
                if isinstance(formatted_col, list):
                    stats_display[col] = formatted_col
                else:
                        print(f"!!!!! ERRO: Tentando atribuir algo que não é uma lista à coluna '{col}' do DataFrame.")

            # Código para imprimir com tabulate (Nível 1)
            try:
                from tabulate import tabulate
                print(f"DEBUG: Tentando imprimir com tabulate...") # DEBUG 14
                print(tabulate(stats_display, headers='keys', tablefmt='grid', showindex=False))
            except ImportError:
                print(f"DEBUG: Tabulate não encontrado. Imprimindo com to_string...") # DEBUG 15
                print("(Instale 'tabulate' para uma tabela mais bonita: pip install tabulate)")
                print(stats_display.to_string(index=False))
            except Exception as e_print:
                    print(f"!!!!! ERRO AO IMPRIMIR TABELA FINAL: {e_print}") # DEBUG 16

        else:
            print("DEBUG: DataFrame de stats é None.") # DEBUG 13
            print("Não foi possível gerar o sumário de estatísticas.")
        # <<< FIM DO CÓDIGO DE DEPURAÇÃO >>>


    def create_visualizations(self, df, timestamp, output_dir):
        # (Mantida igual à versão anterior corrigida)
        ...
        if df.empty or not all(col in df.columns for col in required_cols): missing_cols = [col for col in required_cols if col not in df.columns]; print(f"AVISO: DataFrame vazio ou faltando colunas para gerar gráficos. Faltando: {missing_cols}"); return
        ...
        for metric_key, (title, ylabel, col_name) in metrics_to_plot.items():
             ...
             if col_name == 'flake8_issues': plot_data = plot_data[plot_data[col_name] != -1]
                 ...
                 if col_name in plot_data.columns: plot_data[col_name] = plot_data[col_name] * 100
                 else: print(f"Pulando gráfico de barras '{title}': Coluna {col_name} não encontrada."); continue
             if plot_data.empty or col_name not in plot_data.columns or plot_data[col_name].isnull().all(): print(f"Pulando gráfico de barras '{title}': Nenhum dado válido encontrado após filtragem."); continue
             try:
                 ...
                 if pivot.isnull().all().all(): print(f"Pulando gráfico de barras '{title}': Nenhum dado válido para plotar (tudo NaN)."); plt.close(); continue
                 ax = pivot.plot(kind='bar', figsize=(max(12, num_ais * 2.5), 7), color=[ai_color_map.get(ai, 'grey') for ai in pivot.columns], width=0.8)
        ...
        if 'avg_exec_time_ms' in df_plot.columns: metrics_for_boxplot["avg_exec_time_ms"] = {"title": "Distribuição do Tempo Médio de Execução (Executor) por IA", "ylabel": "Tempo (ms)", "valid_condition": (lambda df, col: df[col].notna() & (df[col] != -1))}
        if 'num_execution_errors' in df_plot.columns: metrics_for_boxplot["num_execution_errors"] = {"title": "Distribuição de Erros de Execução por IA", "ylabel": "Número de Erros", "valid_condition": (lambda df, col: df[col].notna())}
        ...
        for metric_key_boxplot, config in metrics_for_boxplot.items():
            ...
            if temp_df.empty or temp_df[metric_key_boxplot].isnull().all(): print(f"   Pulando Box Plot para '{metric_key_boxplot}': Sem dados válidos após filtro."); continue
            for ai_name in ai_list:
        ...
        if num_ais >= 2:
             ...
             try:
                 numeric_cols_radar = ['api_execution_time', 'lines_of_code', 'completeness']; means_data = {}
                 for ai in ai_list:
                     df_ai_radar = df[df["ai"] == ai]; means = {};
                     for col in numeric_cols_radar: means[col] = df_ai_radar[col].mean() if col in df_ai_radar.columns and not df_ai_radar[col].isnull().all() else 0;
                     means_data[ai] = means
