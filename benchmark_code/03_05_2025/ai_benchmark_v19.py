...
try:
    ...
    try:
        ...
        pkg_resources = None # Define como None se não puder importar
...
class AIBenchmark:
    ...
    def query_gemini(self, prompt: str) -> dict:
        ...
        try:
            ...
            try:
                ...
                # Se response.prompt_feedback ou response.candidates falhar, gerará exceção,
                # que será capturada pelo except externo abaixo.
    ...
    # --- Métodos Execução Local LRU Cache (Correção v18 aplicada) ---
    ...
    def _lru_worker(self, code_string: str, test_sequence: list, result_queue: multiprocessing.Queue):
        ...
        try:
            ...
            safe_builtins = {
                ...
                # REMOVIDO __name__ daqui
            }
            safe_globals = {
                "__builtins__": safe_builtins,
                "OrderedDict": OrderedDict,
                 # --- CORREÇÃO v19: Adicionar __name__ diretamente aqui ---
                "__name__": "__exec_sandbox__"
            }

            # 3. Executa o código da classe
            local_namespace = {}
            # Passa safe_globals como o dicionário global para exec
            ...
            for op_data in test_sequence:
                ...
                try:
                    ...
                    print(f"      Erro passo LRU ({operation}): {step_res['err']}") # Log erro do passo
            ...
            print(f"    ERRO WORKER LRU: {error_message}"); traceback.print_exc(); final_success = False # Log completo
    ...
    def _execute_lru_locally(self, code_string: str, test_sequence: list) -> dict:
        ...
        try:
            ...
            if not terminated:
             ...
             if process and process.is_alive(): try: process.kill(); process.join() 
        except: pass
        finally:
    ...
    def _detect_function_name(self, code: str, challenge: dict) -> str | None:
        ...
        if sig_match:
            ...
            # --- CORREÇÃO v18: Tenta busca simples primeiro ---
            ...
            if re.search(rf"^\s*{re.escape(simple_def)}", code, re.MULTILINE) or \
               ...
               re.search(rf"^\s*{re.escape(simple_class_paren)}", code, re.MULTILINE):
                 # print(f"  Nome esperado '{expected_name}' encontrado (busca simples).") # Log opcional
                 return expected_name

            # Fallback para regex menos restrito (sem ^)
            pattern = rf"\s*(?:def|class)\s+{re.escape(expected_name)}\b"
            exact_match = re.search(pattern, code, re.MULTILINE)
            if exact_match:
                 # print(f"  Nome esperado '{expected_name}' encontrado (regex).") # Log opcional
                 return expected_name
        ...
        if first_match:
            ...
            if not expected_name or found_name != expected_name:
                 # Só avisa se encontrou algo *diferente* do esperado
                 print(f"  AVISO: Usando 1ª definição '{found_name}' (Esperado: '{expected_name or 'N/A'}').")
            return found_name
        return None # Realmente não encontrou nada
    ...
    def run_benchmark(self):
        ...
        for chal in self.code_challenges:
            ...
            for ai in enabled_ais:
                ...
                det=[d for d in self.detailed_test_results if d['ai']==ai and d['challenge']==chal['name']] # Busca detalhes recém-adicionados
    ...
    def _plot_bar_charts(self, df_plot, ai_list, cmap, challenge_order, ts, out_dir):
        ...
        for k,(t,y,c) in metrics.items():
            ...
            try:
                ...
                n_ai=len(ai_list); ax=piv.plot(kind='bar',figsize=(max(12,n_ai*2),7),color=[cmap.get(ai,'grey') for ai in piv.columns],width=0.8)
