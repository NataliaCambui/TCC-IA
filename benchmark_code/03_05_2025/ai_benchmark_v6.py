...
import time
import os
import re # Já estava importado, necessário para a nova lógica
import pandas as pd
import numpy as np
...
class AIBenchmark:
    def __init__(self):
        ...
        # (Mantida igual ao original - omitida aqui para brevidade)
        self.code_challenges = [
             {
                 "name": "Fibonacci Sequence",
                 "description": "Generate the first n numbers in the Fibonacci sequence, starting with 0.",
                 "difficulty": "Easy",
                 "function_signature": "fibonacci(n: int) -> list[int]", # <<< Nota: Assinatura original usa list[int]
                 "test_cases": [
    ...
    # --- Funções query_claude, query_chatgpt, query_gemini (Mantidas iguais ao original) ---
    ...
    # --- Funções de Avaliação (extract_code, count_code_lines, measure_response_completeness, analyze_code_quality_flake8) ---
    # (Mantidas iguais ao original)
    ...
    def analyze_code_quality_flake8(self, code_string):
        ...
        try:
            with tempfile.NamedTemporaryFile(mode='w+', suffix='.py', delete=False, encoding='utf-8') as temp_file:
                temp_file.write(code_string); temp_filepath = temp_file.name
    ...
    # --- Função evaluate_solution MODIFICADA ---
    def evaluate_solution(self, ai_name, challenge, response):
        ...
        actual_function_name = None # <<< NOVO: Nome da função/classe a ser chamada

        # <<< MODIFICADO: Tenta encontrar o nome da função/classe no CÓDIGO EXTRAÍDO >>>
        if code:
            match = re.search(r"^\s*(?:def|class)\s+(\w+)", code, re.MULTILINE)
            if match:
                actual_function_name = match.group(1)
                print(f"   Nome da função/classe detectado no código: '{actual_function_name}'")
            else:
                print(f"   AVISO: Não foi possível detectar nome da função/classe no código extraído para '{challenge['name']}'. Pulando execução.")
        else:
            print(f"   AVISO: Nenhum código extraído para '{challenge['name']}'. Pulando execução.")
        # <<< FIM DA MODIFICAÇÃO >>>

        # Só executa se tivermos código, um nome de função detectado, E não for LRU Cache
        if code and actual_function_name and challenge["name"] != "LRU Cache":
            ...
            if not test_cases:
                ...
                for i, test_case in enumerate(test_cases):
                    ...
                    # <<< MODIFICADO: Chama o serviço executor com o nome detectado >>>
                    ...
                    exec_result["expected_output"] = expected_output
                    execution_results_list.append(exec_result)

        elif challenge["name"] == "LRU Cache":
            # Adiciona um placeholder indicando que não foi executado (mantém lógica original)
        ...
        return {
            ...
            "code": code, # Código extraído (sem os imports adicionados)
            "full_response": content,
            "execution_results": execution_results_list
        }

    # --- Função _call_executor_service MODIFICADA para adicionar imports ---
    def _call_executor_service(self, code_string, function_name, input_args):
        """Envia código e argumentos para o executor service, adicionando imports comuns."""
        executor_url = "https://executor-service-1029404216382.europe-southwest1.run.app/execute" # Sua URL

        # <<< NOVO: Define imports a serem adicionados ao código antes da execução >>>
        # Adiciona imports comuns que causaram problemas (List, re, OrderedDict)
        # e outros tipos úteis do 'typing'.
        imports_to_add = (
        ...
        payload = {
            # <<< MODIFICADO: Envia o código com os imports adicionados >>>
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"   Erro: Chamada ao executor falhou. Status: {response.status_code}, Resposta: {response.text[:200]}...")
    ...
    # --- Função Principal de Execução run_benchmark (Mantida igual ao original) ---
    def run_benchmark(self):
        ...
        for challenge in self.code_challenges:
            ...
            if "class LRUCache" in signature:
                prompt = ( f"Please implement the following Python class based on this description: {challenge['description']}. "
                ...
                prompt = ( f"Please write a single, complete Python function that implements the following signature: `{signature}`. "
            ...
            if can_run_claude:
                ...
                responses["Claude"] = self.query_claude(prompt); time.sleep(1)
            if can_run_openai:
                print("Consultando ChatGPT...")
                responses["ChatGPT"] = self.query_chatgpt(prompt); time.sleep(1)
            if can_run_gemini:
            ...
            for ai_name in enabled_ais:
                ...
                # <<< evaluate_solution agora usa o nome da função detectado e o executor adiciona imports >>>
    ...
    # --- Funções de Geração de Relatório e Visualização generate_report, create_visualizations (Mantidas iguais ao original) ---
    # (Omitidas aqui para brevidade, mas devem ser incluídas no seu arquivo final)
    # A função generate_report já calcula as métricas agregadas corretamente a partir
    # dos execution_results (mesmo que alguns sejam falhas).
    # As funções de visualização usarão essas métricas agregadas.
    def generate_report(self):
         ...
         try:
             ...
             for row_results in df['execution_results']:
                 if not isinstance(row_results, list) or not row_results:
                     ...
                     total_tests_passed.append(0); total_tests_attempted.append(0)
                     continue

                 # <<< MODIFICADO: Considera a CORREÇÃO da saída vs esperado para "passed" >>>
                 # successful_runs = [r for r in row_results if r.get('success') == True] # <-- Linha original baseada apenas na *execução*
                 # Agora verificamos se 'success' é True E se 'output' == 'expected_output'
                 passed_runs = [r for r in row_results if r.get('success') == True and r.get('output') == r.get('expected_output')]
                 # <<< FIM DA MODIFICAÇÃO >>>

                 # Considera tempo apenas se execução ocorreu (success é True ou False, não None)
                 valid_times = [r.get('execution_time_ms') for r in row_results if r.get('execution_time_ms', -1) >= 0 and r.get('success') is not None]

                 num_attempted = len([r for r in row_results if r.get('success') is not None]) # Conta apenas os que tentaram executar
                 # num_passed = len(successful_runs) # <-- Linha original
                 num_passed = len(passed_runs) # <<< USA A NOVA CONTAGEM DE TESTES QUE PASSARAM >>>

                 total_tests_passed.append(num_passed)
                 total_tests_attempted.append(num_attempted) # Mantem como tentativas válidas
             ...
             df['avg_exec_time_ms'] = avg_exec_time_ms
         ...
         try:
             class NpEncoder(json.JSONEncoder):
                 def default(self, obj):
                     ...
                     if isinstance(obj, (datetime,)): return obj.isoformat() # Handle datetime if needed
                     # <<< MODIFICADO: Tratar booleanos numpy se necessário >>>
         ...
         try:
             cols_to_drop_summary = ['code', 'full_response']; summary_cols = [col for col in df.columns if col not in cols_to_drop_summary]
             summary = df[summary_cols]; summary.to_csv(summary_path, index=False, encoding='utf-8', float_format='%.4f')
         ...
         try:
             stats_metrics = [
                 ("Average API Time (s)", "api_execution_time"), ("Average Lines of Code", "lines_of_code"),
                 ("Average Completeness (%)", "completeness"), ("Average Flake8 Issues", "flake8_issues"),
                 ("Average Execution Success (%)", "avg_exec_success_rate"), ("Average Execution Time (ms)", "avg_exec_time_ms"),
                 ("Total Tests Passed", "total_tests_passed"), ("Total Tests Attempted", "total_tests_attempted")
             ]
             ...
             stats = pd.DataFrame(stats_data); stats.to_csv(stats_path, index=False, encoding='utf-8', float_format='%.2f')
    ...
    def create_visualizations(self, df, timestamp, output_dir):
         ...
         ai_list = sorted(df['ai'].unique()); num_ais = len(ai_list)
         ...
         metrics_for_boxplot = {
             "api_execution_time": {"title": "Distribuição do Tempo de Resposta API por IA", "ylabel": "Tempo (s)", "valid_condition": None},
             "lines_of_code": {"title": "Distribuição de Linhas de Código (LoC) por IA", "ylabel": "Número de Linhas", "valid_condition": (lambda df, col: df[col] > 0)},
             "completeness": {"title": "Distribuição da Completude da Solução por IA", "ylabel": "Score (%)", "valid_condition": None},
         }
         ...
         for metric_key_boxplot, config in metrics_for_boxplot.items():
             ...
             if config.get("valid_condition"):
                 ...
                 except Exception as e: print(f"  Aviso: Falha ao aplicar condição para boxplot de '{metric_key_boxplot}': {e}. Continuando sem filtro adicional.")
             if temp_df.empty or temp_df[metric_key_boxplot].isnull().all(): print(f"  Pulando Box Plot para '{metric_key_boxplot}': Sem dados válidos após filtragem."); continue
             for ai_name in ai_list:
             ...
             if not data_for_current_metric: print(f"  Pulando Box Plot para '{metric_key_boxplot}': Nenhum dado encontrado para qualquer IA."); continue
             try:
                 plt.figure(figsize=(max(8, len(labels_for_current_metric) * 1.2), 6))
                 box_plot = plt.boxplot(data_for_current_metric, labels=labels_for_current_metric, patch_artist=True, showfliers=True)
         ...
         if num_ais >= 2:
             ...
             try:
                 ...
                 for ai in ai_list:
                     ...
                     for col in numeric_cols_radar: means[col] = df_ai_radar[col].mean() if col in df_ai_radar.columns else 0
                     means_data[ai] = means
                 ...
                 valid_locs = [loc for loc in all_locs if loc > 0]; min_loc_valid = min(valid_locs) if valid_locs else 0
                 max_loc_valid = max(all_locs + [1e-6]); lines_norm = {}
                 for ai in ai_list:
