...
class AIBenchmark:
    ...
    def _plot_bar_charts(self, df_plot, ai_list, cmap, challenge_order, ts, out_dir):
        ...
        for key, (title, ylabel, col_name) in metrics_def.items():
            ...
            try:
                ...
                pivot_table.fillna(fill_value=fill_val,inplace=True)
    ...
    def _plot_radar_chart(self, df, ai_list, cmap, ts, out_dir):
        ...
        # Versão simplificada e mais legível
        num_ais = len(ai_list)
        if num_ais < 2: print("Radar pulado (< 2 IAs)."); return
        print(f"Gerando radar p/ {num_ais} IAs..."); cols=['api_execution_time','lines_of_code','completeness']
        try:
            if not all(c in df.columns for c in cols): print("Skip Radar: Faltando cols."); return
            # Calcula médias
            means_data = {}
            for ai in ai_list:
                df_ai=df[df["ai"]==ai]; means = {}
                for c in cols: means[c]=df_ai[c].mean() if not df_ai[c].isnull().all() else 0
                means_data[ai]=means

            # Normaliza
            all_times=[m.get(cols[0],0) for m in means_data.values()]; max_t=max(all_times+[1e-6])
            time_norm={ai:100*(1-means_data[ai].get(cols[0],0)/max_t) if max_t>0 else 0 for ai in ai_list}
            all_locs=[m.get(cols[1],0) for m in means_data.values()]; v_locs=[l for l in all_locs if l>0]; min_l=min(v_locs) if v_locs else 0; max_l=max(all_locs+[1e-6])
            lines_norm={}
            for ai in ai_list:
                loc=means_data[ai].get(cols[1],0)
            ...
            comp_norm={ai: means_data[ai].get(cols[2],0) for ai in ai_list}

            # Plota
            categories=['Veloc API(Norm)','Conc LoC(Norm)','Completude(%)']; N=len(categories)
            angles=np.linspace(0, 2 * np.pi, N, endpoint=False).tolist(); angles+=ang[:1]
            fig, ax = plt.subplots(figsize=(9,9), subplot_kw=dict(polar=True)) # Cria figura/eixos
            for ai in ai_list:
                 vals=[time_norm.get(ai,0),lines_norm.get(ai,0),comp_norm.get(ai,0)]
                 plot_values=vals+[vals[0]] # Fecha polígono
                 color=cmap.get(ai,'grey')
                 ax.plot(angles,plot_values,'o-',linewidth=2,label=ai,color=color)
