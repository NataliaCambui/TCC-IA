...
class AIBenchmark:
        ...
        try:
            ...
            if result.returncode in [0, 1] and result.stdout:
                try:
                    ...
                    # Fallback: Se não conseguir parsear o número, conta as linhas
    ...
    def _lru_worker(self, code_string: str, test_sequence: list, result_queue: multiprocessing.Queue):
        ...
        try:
            ...
            safe_builtins = {
                ...
                'len': len, 'range': range, 'print': print, # print para debug
                ...
                # Built-ins seguros essenciais
            }
            safe_globals = {
                "__builtins__": safe_builtins,
                "OrderedDict": OrderedDict # Passa explicitamente o necessário
            }

            # 2. Executa o código da classe para defini-la
            local_namespace = {}
            exec(code_string, safe_globals, local_namespace)

            # 3. Encontrar o nome da classe
            class_name = None
            ...
            if not class_name:
                ...
                if match:
                    ...
                    raise NameError("Não foi possível encontrar o nome da classe LRUCache no código.")
            ...
            for op_data in test_sequence:
                ...
                try:
                    if operation == "__init__":
                        ...
                        if str(output) == str(expected): # Comparação simples
                            step_result["success"] = True
                        else:
    ...
    def _execute_lru_locally(self, code_string: str, test_sequence: list) -> dict:
        ...
        """
        ctx = multiprocessing.get_context(None) # ou 'spawn'
    ...
    def evaluate_solution(self, ai_name: str, challenge: dict, response: dict) -> dict:
        ...
        execution_results_list = [] # Resultados brutos (passos LRU ou executor)
        processed_test_details = [] # Formatado para CSV detalhado
        num_execution_errors = 0    # Conta falhas/erros nos test cases
        first_execution_error = ""  # Primeira mensagem

        # Precisa do código para executar, exceto se for detetar nome de função
        ...
        if can_attempt_execution:
            ...
            if challenge["name"] == "LRU Cache":
                ...
                if test_cases and isinstance(test_cases[0], dict) and "sequence" in test_cases[0]:
                    # Chama a execução local
                    lru_exec_result = self._execute_lru_locally(code, test_cases[0]["sequence"])

                    execution_results_list = lru_exec_result.get("results", []) # Guarda detalhes dos passos
                    ...
                    first_execution_error = lru_exec_result.get("error", "") # Erro geral
                    ...
                    # Cria entrada única para detailed_test_results
                    avg_step_time = -1.0
                    ...
                    processed_test_details.append({
                        ...
                        "execution_time_ms": avg_step_time,
                        "error_message": first_execution_error[:250], # Limita tamanho
                        "stdout": None
                    })
                    print(f"  Resultado execução local LRU: {'Sucesso' if overall_success else 'Falha/Erro'}")

                else: # Caso não encontre a sequência de teste
            ...
            # --- Fim da Lógica LRU Cache ---
            ...
            else: # Executa outros desafios via serviço externo (actual_function_name deve existir)
                ...
                processed_test_details.extend(processed_details_external)
            # --- Fim da Lógica de outros desafios ---

        else: # Caso não haja código ou nome de função detectado (e não seja LRU)
            print(f"  AVISO: Nenhum código válido ou nome de função detectado para "
                  f"'{challenge['name']}' por {ai_name}. Pulando execução.")
            # Adiciona entradas de erro para cada caso de teste esperado
            num_tests = len(challenge.get("test_cases", []))
            error_msg = "No code extracted" if not code else "Function name not detected"
            ...
            num_execution_errors = num_tests # Conta todos como erro
            first_execution_error = error_msg

        # Adiciona os detalhes formatados à lista global
        # (Isto foi movido para dentro dos blocos if/else/elif acima para garantir que é adicionado)
        # self.detailed_test_results.extend(processed_test_details) <== REMOVIDO DAQUI

        # Retorna o dicionário de resultados agregados
        return {
            "ai": ai_name,
    ...
    def _detect_function_name(self, code: str, challenge_name: str) -> str | None:
        ...
        if match:
            ...
            # Não imprime aviso aqui, deixa evaluate_solution decidir o que fazer
    ...
    def _run_test_cases(self, ai_name: str, challenge: dict, code: str,
        ...
        num_failures_or_errors = 0 # Renomeado para clareza
        first_error_message = ""   # Renomeado para clareza
        test_cases = challenge.get("test_cases", [])
        ...
        for i, test_case in enumerate(test_cases):
            ...
            current_case_error = None # Erro para este caso específico
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    print(f"  Erro: Falha ao decodificar JSON executor. Status: {response.status_code}, Erro: {json_err}")
                    default_error_result["error"] = f"Executor response JSON decode error: {json_err}"
                    return default_error_result
            else:
                error_text = response.text[:200]
                print(f"  Erro: Chamada ao executor falhou. Status: {response.status_code}, Resp: {error_text}...")
            ...
            print(f"  Erro: Conexão/rede ao chamar o executor service: {req_err}")
    ...
    def run_benchmark(self):
        ...
        print(f"  LRU Cache Execution: Local Process") # Informa execução local
        print("==========================================")
        ...
        for challenge in self.code_challenges:
            ...
            if "class LRUCache" in signature:
                 prompt = (
                     ...
                     f"(like `from collections import OrderedDict`). Do not include any explanations, "
                     f"example usage, markdown formatting (like ```python), or introductory sentences."
                 )
            else:
                 prompt = (
            ...
            for ai_name in enabled_ais:
                ...
                # Chama evaluate_solution (agora executa LRU localmente)
                ...
                # Filtra detalhes de teste relevantes
                ...
                # Para LRU, mostra apenas Pass/Fail baseado no resultado agregado
                elif challenge['name'] == "LRU Cache":
                     exec_status = "Passed" if tests_error_fails == 0 else f"Failed ({tests_error_fails} error/fail)"
                else:
                     exec_status = f"{tests_passed}/{tests_attempted} passed"

                summary_line = (f"  {ai_name:<10}: Exec: {exec_status}. "
                                f"Flake8: {flake8_str}. "
    ...
    # Nenhuma alteração necessária aqui, usam os dados processados
    ...
    def _calculate_aggregated_test_metrics(self) -> dict:
        ...
        for key in result_keys:
            ...
            # LRU tem 1 'detail', usa seu tempo médio dos passos. Outros usam tempos válidos.
            if challenge_name == "LRU Cache":
                 ...
                 valid_times = [d['execution_time_ms'] for d in details
                                ...
                                d['status'] not in ['Unknown', 'Not Executed', 'No Code', 'Error']]
            ...
            with open(env_info_path, "w", encoding='utf-8') as f:
                for key, value in self.environment_info.items():
                    if isinstance(value, dict):
                        f.write(f"{key}:\n")
                        for sk, sv in value.items(): f.write(f"  {sk}: {sv}\n")
    ...
    def _create_csv_header(self) -> str:
        ...
        for key, value in self.environment_info.items():
            if isinstance(value, dict):
                ...
                for sk, sv in value.items(): env_comments.append(f"#   {sk}: {sv}")
    ...
    def _calculate_statistics(self, df: pd.DataFrame) -> pd.DataFrame | None:
         try:
             stats_metrics = [
                 ...
                 ("Average Flake8 Issues", "flake8_issues", "mean_valid"),
                 ...
                 ("Average Execution Time (ms)", "avg_exec_time_ms", "mean_valid"),
                 ("Total Execution Errors/Fails", "num_execution_errors", "sum"), # Nome da métrica
                 ("Example First Error", "first_execution_error", "first_non_empty")
             ]
             ai_list = sorted(df['ai'].unique())
             stats_data = {"Metric": [m[0] for m in stats_metrics]}
             for ai_name in ai_list:
                 df_ai = df[df["ai"] == ai_name]
                 ai_column_data = []
                 for metric_name, col_key, agg_type in stats_metrics:
                     if col_key not in df_ai.columns:
                         ai_column_data.append(np.nan)
                         continue
    ...
    def _display_summary_table(self, stats_df: pd.DataFrame):
        ...
        for col in stats_display.columns:
            ...
            for i in stats_display.index:
                ...
                if not pd.isna(value):
                    ...
                    elif "Total Tests" in metric_name or "Total Execution Errors/Fails" in metric_name: formatted_val = f"{int(value)}"
        ...
        try:
            ...
            print(stats_display.to_string(index=False)) # Fallback
    ...
    def generate_report(self):
        ...
        if df is None:
            print("Falha ao preparar DataFrame. Relatórios subsequentes abortados.")
            return
    ...
    def _plot_bar_charts(self, df_plot: pd.DataFrame, ai_list: list, ai_color_map: dict,
        ...
        metrics_to_plot = {
            ...
            "7_num_execution_errors": ("Média Erros/Falhas Execução", "Número de Erros/Falhas", "num_execution_errors"),
        }
        ...
        if 'avg_exec_time_ms' in df_plot.columns: metrics_to_plot["6_avg_exec_time_ms"] = ("Tempo Médio de Execução", "Tempo (ms)", "avg_exec_time_ms")
        ...
        for metric_key, (title, ylabel, col_name) in metrics_to_plot.items():
            if col_name not in df_plot.columns:
                 print(f"Pulando gráfico '{title}': Coluna {col_name} não encontrada.")
                 continue
            ...
            try:
                ...
                if pivot.isnull().all().all():
                    print(f"Pulando gráfico '{title}': Nenhum dado válido (tudo NaN).")
                    plt.close(); continue
                num_ais = len(ai_list)
                ...
                if col_name in ['completeness', 'avg_exec_success_rate']: plt.ylim(0, 105)
                    ...
                    max_val = pivot.max().max(); plt.ylim(bottom=0, top = max(1, max_val * 1.1) if not pd.isna(max_val) else 1)
            ...
            except Exception as e: print(f"Erro ao gerar gráfico '{title}': {e}\nTraceback: {traceback.format_exc()}"); plt.close()
    ...
    def _plot_box_plots(self, df_plot: pd.DataFrame, ai_list: list, ai_color_map: dict,
        ...
        if 'avg_exec_time_ms' in df_plot.columns: metrics_for_boxplot["avg_exec_time_ms"] = {"title": "Distribuição Tempo Médio Execução por IA", "ylabel": "Tempo (ms)", "valid_condition": (lambda df, col: df[col].notna() & (df[col] != -1))}
        if 'num_execution_errors' in df_plot.columns: metrics_for_boxplot["num_execution_errors"] = {"title": "Distribuição Erros/Falhas Execução por IA", "ylabel": "Número de Erros/Falhas", "valid_condition": (lambda df, col: df[col].notna())}
        ...
        for metric_key_boxplot, config in metrics_for_boxplot.items():
            ...
            if metric_key_boxplot not in df_plot.columns:
                print(f"  Pulando Box Plot '{config['title']}': coluna não encontrada.")
                continue
...
if __name__ == "__main__":
    ...
    try:
        ...
        if benchmark.claude_api_key or benchmark.openai_api_key or benchmark.gemini_api_key:
            ...
            print("\nERRO FATAL: Nenhuma chave de API válida config/encontrada. Benchmark não pode ser executado.")
